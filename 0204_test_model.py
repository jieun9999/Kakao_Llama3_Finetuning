...
# 테스트 프롬프트
input_text = "role: user, speechAct: 주장하기, content: 와 저녁 되니까 야식 땡긴다 ㅋㅋ"
...
try:
    ...
    # # 정규식을 사용하여 첫 번째 role: 로 시작하는 패턴 추출
    # pattern = r"role:\s*[^\s]+"  # role:로 시작하는 텍스트 추출
    # match = re.search(pattern, result)  # 첫 번째 일치 항목만 추출
    # if match:
    #     extracted_text = match.group(0)  # 첫 번째 매칭된 텍스트 가져오기
    #     print("결과:", extracted_text)
    # else:
    #     print("결과에서 패턴을 찾을 수 없습니다.")
