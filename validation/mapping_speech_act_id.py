...
def modify_lines_with_speech_act_and_role(base_folder_path, target_folder):
    folder_path = os.path.join(base_folder_path, target_folder)  # 각 KAKAO 폴더 경로 설정
            ...
            if filename.endswith(".json"):  # JSON 파일만 처리
                ...
                with open(file_path, 'r', encoding='utf-8') as file:
                    try:
                        ...
                        print(f"Error decoding JSON in file: {target_folder}/{filename}")

# 실행
target_folder = "KAKAO"  # 순차적으로 탐색할 폴더 목록

modify_lines_with_speech_act_and_role(base_folder_path, target_folder)